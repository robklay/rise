...
City::City(const std::string& name, const rok::Coordinate position, const rok::int64 population, const World* world) :
_world(world),
_name(name),
_position(position),
...
rok::Coordinate City::position() const {
	return _position;
}
...
void City::draw(sf::RenderTarget& target, sf::RenderStates states) const {
	...
	_city_text.setOrigin(_city_text.getLocalBounds().width / 2.0f,
	                     _city_text.getLocalBounds().height / 2.0f);
	_city_text.setPosition(static_cast<float>(_position.x),
